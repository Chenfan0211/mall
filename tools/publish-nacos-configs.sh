...
until curl -fsS "$NACOS_ADDR/nacos/" >/dev/null; do sleep 2; done
...
curl -fsS "$NACOS_ADDR/nacos/v1/cs/configs?tenant=${NAMESPACE_ID}&group=${GROUP}&dataId=mall-common.yaml" >/tmp/nacos_common_check.yaml
