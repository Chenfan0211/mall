  ...
  <main class="login-page">
    <section class="login-card">
      <div class="login-copy">
        <span class="eyebrow">Warehouse console</span>
        <h1>WMS 仓库后台</h1>
        <p>围绕入库、库存、出库、配送和回仓处理仓库作业。</p>
      </div>
      <el-form class="login-form" :model="form" label-position="top" @submit.prevent="submit">
        <el-form-item label="账号">
          <el-input v-model="form.username" placeholder="请输入仓库后台账号" />
        </el-form-item>
...
async function submit() {
    const result = await passwordLogin({ ...form, portalCode: 'wms-admin' });
